...
namespace neo_raknet.Packet.MinecraftPacket
{
    ...
    public class McpeItemRegistry : Packet
    {
        ...
        protected override void DecodePacket()
        {
            ...
            uint count = ReadUnsignedVarInt(); // 读取物品数量 (uint)
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                item.RuntimeID = ReadShort(false);    // 读取运行时 ID (short, 假设小端)
                item.ComponentBased = ReadBool();     // 读取是否基于组件 (bool)
                item.Version = ReadInt(false);        // 读取版本 (int, 假设小端)
