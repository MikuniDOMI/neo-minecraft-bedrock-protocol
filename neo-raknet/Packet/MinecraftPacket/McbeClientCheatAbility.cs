...
using neo_raknet.Packet.MinecraftStruct; // Assuming AbilityData, AbilityLayer, etc. are in this namespace
...
namespace neo_raknet.Packet.MinecraftPacket
{
    ...
    public class McpeClientCheatAbility : Packet
    {
        ...
        /// </summary>
        public AbilityData AbilityData { get; set; } = new AbilityData(); // Initialize with default values
        ...
        private AbilityData ReadAbilityData()
        {
            ...
            long entityUniqueID = ReadLong(); // methods.txt: long ReadLong()

            // Read PlayerPermissions (uint8 -> byte)
            byte playerPermissions = ReadByte(); // methods.txt: byte ReadByte()

            // Read CommandPermissions (uint8 -> byte)
            byte commandPermissions = ReadByte(); // methods.txt: byte ReadByte()

            // Read Layers ([]AbilityLayer)
            // This corresponds to protocol.Slice(io, &pk.Layers)
            uint layersCount = ReadUnsignedVarInt(); // Read length as Varuint32 (methods.txt: uint ReadUnsignedVarInt())
            AbilityLayer[] layers = new AbilityLayer[layersCount];
