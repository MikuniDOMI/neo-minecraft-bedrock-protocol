using System;
using System.Collections.Generic; // For map[string]any -> Dictionary<string, object>
using neo_raknet.Packet; // Assuming base Packet class is here or adjust accordingly
using neo_raknet.Packet.MinecraftStruct.NBT; // Assuming Nbt and related types are here or adjust accordingly
...
namespace neo_raknet.Packet.MinecraftPacket
{
    ...
    public class McpeEditorNetwork : Packet
    {
        ...
        /// 在 C# 中，Go 的 map[string]any 通常表示为 Dictionary<string, object>。
        /// 假设 Nbt 类型（或类似类型）可以表示这种动态的 NBT 复合标签。
        /// </summary>
        public Nbt Payload { get; set; } // map[string]any -> Nbt (or Dictionary<string, object> if Nbt type cannot hold compound directly)
