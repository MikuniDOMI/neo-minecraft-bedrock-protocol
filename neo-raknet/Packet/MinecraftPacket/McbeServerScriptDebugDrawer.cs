...
namespace neo_raknet.Packet.MinecraftPacket
{
    ...
    namespace neo_raknet.Protocol // Or your relevant namespace
    {
        ...
        public enum ScriptDebugShapeType :
            byte // Go's iota defaults to int, but uint8 is specified in DebugDrawerShape.Type
        {
            /// <summary>
        ...
        public class DebugDrawerShape
        {
            ...
            /// </summary>
            public Optional<int> Colour { get; set; } // protocol.Optional[color.RGBA] -> Optional<Color>
    ...
    public class McbeServerScriptDebugDrawer : Packet
    {
        ...
        public List<DebugDrawerShape> Shapes { get; set; } = new(); // List of shapes to be drawn by the debug drawer
        ...
        protected override void EncodePacket()
        {
            ...
            foreach (var shape in Shapes)
            {
                ...
                if (shape.Type.HasValue) Write(shape.Type.Value); // Write the byte value

                // Write Location (Optional<Vector3>)
                Write(shape.Location.HasValue);
                if (shape.Location.HasValue) Write(shape.Location.Value); // Assumes Write(Vector3) exists
                Write(shape.Scale.HasValue);
                if (shape.Scale.HasValue) Write(shape.Scale.Value); // Write float

                // Write Rotation (Optional<Vector3>)
                Write(shape.Rotation.HasValue);
                if (shape.Rotation.HasValue) Write(shape.Rotation.Value); // Assumes Write(Vector3) exists

                // Write TotalTimeLeft (Optional<float32> -> Optional<float>)
                Write(shape.TotalTimeLeft.HasValue);
                if (shape.TotalTimeLeft.HasValue) Write(shape.TotalTimeLeft.Value); // Write float
                ...
                if (shape.Text.HasValue) Write(shape.Text.Value); // Write string

                // Write BoxBound (Optional<Vector3>)
                Write(shape.BoxBound.HasValue); // Write bool indicating if BoxBound is set
                if (shape.BoxBound.HasValue) Write(shape.BoxBound.Value); // Assumes Write(Vector3) exists
                ...
                if (shape.LineEndLocation.HasValue) Write(shape.LineEndLocation.Value); // Assumes Write(Vector3) exists
                ...
                if (shape.ArrowHeadLength.HasValue) Write(shape.ArrowHeadLength.Value); // Write float
                ...
                if (shape.ArrowHeadRadius.HasValue) Write(shape.ArrowHeadRadius.Value); // Write float

                // Write Segments (Optional<byte>)
                Write(shape.Segments.HasValue);
                if (shape.Segments.HasValue) Write(shape.Segments.Value); // Write byte
            }
        }
        ...
        protected override void DecodePacket()
        {
            ...
            for (var i = 0; i < count; i++)
            {
                ...
                var hasType = ReadBool(); // methods.txt: bool ReadBool()
                if (hasType) shape.Type = new Optional<byte>(ReadByte()); // methods.txt: byte ReadByte()
                ...
                if (hasScale) shape.Scale = new Optional<float>(ReadFloat()); // methods.txt: float ReadFloat()
                ...
                if (hasColour)
                    ...
                    shape.Colour = new Optional<int>(ReadInt()); // Placeholder - Replace with actual method
                ...
                if (hasText) shape.Text = new Optional<string>(ReadString()); // methods.txt: string ReadString()
                ...
                if (hasSegments) shape.Segments = new Optional<byte>(ReadByte()); // methods.txt: byte ReadByte()
            }
        }
    }
}