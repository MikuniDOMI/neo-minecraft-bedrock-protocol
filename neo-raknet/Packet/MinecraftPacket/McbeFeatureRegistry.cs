...
namespace neo_raknet.Packet.MinecraftPacket
{
    ...
    public class McpeFeatureRegistry : Packet
    {
        ...
        /// </summary>
        public GenerationFeature[] Features { get; set; } = new GenerationFeature[0]; // []protocol.GenerationFeature -> GenerationFeature[]
        ...
        protected override void DecodePacket()
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                // 因此需要在此类中手动反序列化其字段。
                string name = ReadString();        // Read string (methods.txt: string ReadString())
                byte[] json = ReadByteArray(true); // Read byte[] (methods.txt: byte[] ReadByteArray(bool slurp))
