...
public class McpeCameraAimAssistPresets : Packet
{
    ...
    public CameraAimAssistCategory[] Categories { get; set; } =
        new CameraAimAssistCategory[0]; // []protocol.CameraAimAssistCategory -> array/list of structs

    /// <summary>
    ///     Presets 是一个预设列表，定义了瞄准辅助行为的基础。
    /// </summary>
    public CameraAimAssistPreset[] Presets { get; set; } =
        new CameraAimAssistPreset[0]; // []protocol.CameraAimAssistPreset -> array/list of structs
    ...
    private void Write(CameraAimAssistPriorities priorities)
    {
        ...
        if (priorities.EntityDefault.HasValue) Write(priorities.EntityDefault.Value); // Write int

        // Write BlockDefault (Optional<int>)
        Write(priorities.BlockDefault.HasValue); // Write bool
        if (priorities.BlockDefault.HasValue) Write(priorities.BlockDefault.Value); // Write int
    }
    ...
    private void Write(CameraAimAssistPreset preset)
    {
        ...
        if (preset.HandSettings.HasValue) Write(preset.HandSettings.Value ?? string.Empty); // Write string
    }
    ...
    private CameraAimAssistPriorities ReadCameraAimAssistPriorities()
    {
        ...
        if (hasEntityDefault) entityDefault = new Optional<int>(ReadInt()); // Read int32, assuming little-endian

        // Read BlockDefault
        var hasBlockDefault = ReadBool();
        var blockDefault = new Optional<int>();
        if (hasBlockDefault) blockDefault = new Optional<int>(ReadInt()); // Read int32, assuming little-endian
    ...
    private CameraAimAssistPriority ReadCameraAimAssistPriority()
    {
        ...
        var priority = ReadInt(); // Read int32, assuming little-endian
